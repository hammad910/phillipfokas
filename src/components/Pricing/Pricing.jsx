  ...
  return (
    <section className={styles.section}>
      <div className={styles.container}>
        <h2 className={styles.heading} style={{ color: '#fff' }}>Early Adopter Pricing Plans</h2>
        <p className={styles.subheading}>Pay only $65 today, complete payment on August 31st with massive discounts + holding deposit credit.</p>
        <div className={styles.grid}>
          {plans.map((p) => (
            <div key={p.name} className={`${styles.card} ${p.featured ? styles.featured : ''}`}>
              <div className={styles.planName} style={{ fontSize: '30px' }}>{p.name}</div>
              ...
              <a href="#reserve" className={`${styles.cta} ${p.featured ? styles.ctaPrimary : ''}`}>{p.cta}</a>
            </div>
          ))}
        </div>
      </div>
